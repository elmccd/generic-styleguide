...
const getResponse = function ({theme, page, context, segments, parsers = {}, extensions = {}}) {

    console.log('Building page:', theme + '/' + segments.join('/'));

    const template = getThemeTemplate(theme, context, extensions);

    let rawContent = getResponseRawContent(page, parsers);
    let content = getResponseContent(page, parsers, extensions, rawContent);
    let extensionParser = getExtensionParser(template.data.entry, extensions);
...
const findPage = (segments, pages) => {
    ...
    if (segments && segments.length) {
        ...
        let [segment, ...restSegments] = segments;
...
const serve = function serve (port, contextObj) {
    ...
    app.get('/:theme?/*', function (req, res) {
        ...
        if (req.params.theme && !isTheme(req.params.theme, context)) {
            res.sendStatus(404);
            return;
        }
...
module.exports = function Primer (contextObject) {
    ...
    this.build = function (buildPath, themesIds) {
        ...
        const { extensions, parsers } = contextObject;

        buildSetup(buildPath, context.themesDir);

        //show only enabled themes
        context.themes = context.themes.filter(theme => themesIds.includes(theme.id));

        const buildPage = (page, segments, themeId, defaultFilePath) => {
            const filePath = defaultFilePath || path.join(...[buildPath, themeId, ...segments,  'index.html']);
        ...
        const buildPages = (page, segments, themeId) => {
            ...
            if (!page.pages) {
                ...
                if (firstPage) {
                    buildPage(
                        firstPage, 
                        [...segments, firstPage.id], 
                        themeId, 
                        path.join(...[buildPath, themeId, ...segments,  'index.html'])
                    );
                }
                
                //build all children pages
                page.pages.forEach(subPage => {
                    buildPages(subPage, [...segments, subPage.id], themeId);
                });
            }
